...
//go:embed zk-single-kafka-multiple.yml
var KafkaComposeFile string

func KafkaCompose(ctx context.Context, wg *sync.WaitGroup) (kafkaUrls []string, err error) {
	log.Println("kafka docker-compose up")
	stack, err := compose.NewDockerComposeWith(compose.WithStackReaders(strings.NewReader(KafkaComposeFile)))
	if err != nil {
		return kafkaUrls, err
	}
	...
	go func() {
		defer wg.Done()
		<-ctx.Done()
		log.Println("kafka docker-compose down", stack.Down(context.Background(),
	...
	kafkaUrls = []string{hostIp + ":9092", hostIp + ":9093", hostIp + ":9094"}

	err = retry(1*time.Minute, func() error {
		return tryKafkaConn(kafkaUrls[0])
	})
	if err != nil {
		return kafkaUrls, err
	}
